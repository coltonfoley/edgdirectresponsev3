...
const description = descArg
  ? descArg.split('=')[1]
  : `Premium outdoor living covering ${name}.`;
const communities = communitiesArg
  ? communitiesArg.split('=')[1].split(',')
  : [name];
  ...
  if (!pageContent.includes(`slug: "${slug}"`)) {
    ...
    if (arrayEndRegex.test(pageContent)) {
      ...
      if (endOfArray > -1) {
        ...
        console.warn(
          'Could not find insertion point in service-areas/page.tsx'
        );
      }
    }
  } else {
