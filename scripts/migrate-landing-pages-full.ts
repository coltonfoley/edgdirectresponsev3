/**
 * Full Landing Page Migration Script
 * Populates ALL sections: hero, problem, solution, whatYouGet, process, testimonials, FAQs, caseStudy, ROI
 */
...
// ==================== DESIGN PAGE DATA ====================
const designPageData = {
  title: 'Design Consultation - EDG',
  pageType: 'design',
  hero: {
    badge: 'For Homeowners Who Want It Done Right',
    headline: [
      { _type: 'block', _key: generateKey(), style: 'normal', children: [{ _type: 'span', _key: generateKey(), text: '$40,000 outdoor projects fail every day. Yours doesn\'t have to.' }] }
    ],
  ...
  solutionSection: {
    ...
    image: {
      ...
      caption: 'A custom outdoor living space with retractable glass walls in Barrington, IL'
    }
  },
  whatYouGet: {
...
const pricePageData = {
  ...
  hero: {
    ...
    headline: [
      { _type: 'block', _key: generateKey(), style: 'normal', children: [{ _type: 'span', _key: generateKey(), text: 'Get a real quote. Not a bait-and-switch estimate.' }] }
    ],
...
const proPageData = {
  ...
  hero: {
    ...
    headline: [
      { _type: 'block', _key: generateKey(), style: 'normal', children: [{ _type: 'span', _key: generateKey(), text: 'Build faster. Deliver better outdoor living.' }] }
    ],
  ...
  solutionSection: {
    ...
    image: {
      ...
      caption: 'Builder project in North Shore, completed on schedule'
    }
  },
  tradeServices: {
  ...
  caseStudy: {
    ...
    testimonial: {
      quote: 'EDG made shading the easiest part of the project. No delays, no surprises, no callbacks. That\'s rare in this business.',
      name: 'Tom Reynolds',
      title: 'Reynolds Custom Homes, Barrington'
    }
  },
  guarantee: {
    icon: 'Shield',
...
const commercialPageData = {
  ...
  hero: {
    ...
    headline: [
      { _type: 'block', _key: generateKey(), style: 'normal', children: [{ _type: 'span', _key: generateKey(), text: 'Turn your patio into a profit center.' }] }
    ],
  ...
  solutionSection: {
    ...
    image: {
      url: '/images/commercial/restaurant-shade-hero.jpg',
      caption: 'Restaurant patio in Lake Geneva, WI'
    }
  },
  industries: {
  ...
  roiCaseStudy: {
    ...
    testimonial: {
      quote: 'The pergola paid for itself by July. Our patio went from a liability to our single biggest revenue driver. We booked $180K in additional covers the first summer alone.',
      name: 'Restaurant Owner',
      title: 'Name withheld for privacy'
    },
    roiTime: '4 months'
  },
  capabilities: {
...
// Helper to get landing page ID by type
async function getLandingPageId(pageType: string): Promise<string | null> {
  const query = `*[_type == "landingPage" && pageType == $pageType][0]._id`;
  const result = await client.fetch(query, { pageType });
  return result || null;
}

// Update landing page with full data
async function updateLandingPage(pageType: string, data: any) {
  const pageId = await getLandingPageId(pageType);
  
  if (!pageId) {
    console.error(`Landing page with type "${pageType}" not found. Creating...`);
    // Create new document
    const newDoc = {
      _type: 'landingPage',
      title: data.title,
      slug: { _type: 'slug', current: pageType },
      pageType: pageType,
      ...data
    };
  ...
  console.log(`✓ Updated ${pageType} landing page: ${pageId}`);
}
...
async function migrateAllLandingPages() {
  ...
  try {
    console.log('Migrating Design page...');
    await updateLandingPage('design', designPageData);
    
    console.log('Migrating Price page...');
    await updateLandingPage('price', pricePageData);
    
    console.log('Migrating Pro page...');
    await updateLandingPage('pro', proPageData);
    
    console.log('Migrating Commercial page...');
    await updateLandingPage('commercial', commercialPageData);
    
    console.log('\n✅ All landing pages migrated successfully!');
  } catch (error) {
