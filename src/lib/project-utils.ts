 ...
 * @param existingSlugs - Array of existing slugs to ensure uniqueness
...
export function transformCSVRow(
  ...
  const title = row['Project Name']?.trim();
  const address = row['Address']?.trim();
  const description = row['Description (1-2 sentence summary for the project card)']?.trim();
  const challenge = row['Challenge (A paragraph describing the client\'s problem/need)']?.trim();
  const solution = row['Solution (A paragraph describing the work you did)']?.trim();
  const resultsText = row['Results (3-4 bullet points of specific outcome)']?.trim();
  ...
  const id = slugifyUnique(title, existingSlugs);

  // Parse location
  const { city, state } = extractCityState(address || '');
  if (!city || !state) {
    warnings.push(`Could not parse city/state from address: ${address}`);
  }
  ...
  // Detect systems from description and title
  const systems = detectSystems(`${title} ${description || ''}`);
  ...
  if (address) {
    ...
    if (parsed.streetAddress) location.streetAddress = parsed.streetAddress;
    if (parsed.zipCode) location.zipCode = parsed.zipCode;
  }
  ...
  const project: Project = {
    ...
    images: {
      hero: {
        src: `/images/projects/${id}/hero.jpg`,
        alt: `${title} - Main project view`,
      },
      card: {
        src: `/images/projects/${id}/card.jpg`,
  ...
  project.dataCompleteness = calculateDataCompleteness(project);

  // Generate service area slug
  if (city && state) {
    project.serviceAreaSlug = `${slugify(city)}-${state.toLowerCase()}`;
  }
...
/**
 * Imports multiple CSV rows into Project objects
 * 
 * @param rows - Array of CSV rows
 * @returns Import result with success/failure tracking
 */
export function importProjectsFromCSV(
  ...
  warnings: { row: number; projectId: string; warning: string }[];
} {
  const success: Project[] = [];
  const failed: { row: number; error: string }[] = [];
  const warnings: { row: number; projectId: string; warning: string }[] = [];
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      const project = transformCSVRow(row, rowNumber, existingSlugs);

      if (project) {
        success.push(project);
        existingSlugs.push(project.id);

        // Collect warnings
        if (project.importMetadata?.warnings) {
          for (const warning of project.importMetadata.warnings) {
            warnings.push({
              row: rowNumber,
